...
// TODO: Make sure I have all of the rules for NFA construction and
// transitioning down correctly.
...
NFA test_run() {
  ...
  std::vector<std::string> transitionStrings{"0 eps 1", "1 eps 2", "2 eps 3",
                                             "3 eps 4"};
...
void populateStateTransitions(const NFA &n,
                              std::vector<std::shared_ptr<State>> &states) {
      ...
      for (int j = 0; j < testStrings[i].length(); ++j) {
        ...
        for (int x = 0; x < validStates.size(); ++x) {
          ...
          for (auto transition : validStates[x]->transition_states) {
            // if (validStates[x]->transition_states[y].symbol ==
            // asString(testStrings[i][j]))
            if (transition.symbol == asString(testStrings[i][j])) {
              ...
              for (int z = 0; z < nextValidStates.size(); ++z) {
                if (nextValidStates[z]->state == transition.to_state->state) {
                  stateAlreadyInNextValidStates = true;
                }
              }
              if (!stateAlreadyInNextValidStates) {
                auto epsilonExpandedNextState{
                    performStateEpsilonExpansion(transition.to_state, states)};
                nextValidStates = appendUniqueStates(nextValidStates,
                                                     epsilonExpandedNextState);
              }
            }
          }
        }
